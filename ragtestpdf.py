...
Upload TEXT files or PDFs and ask questions about them.
...
class SimpleRAG:
    ...
    def smart_chunk_text(self, text, chunk_size=600, overlap=100):
        ...
        for sentence in sentences:
            ...
            if len(current_chunk) + len(sentence) > chunk_size and current_chunk:
                ...
                overlap_words = ' '.join(words[-overlap//5:])  # Roughly 'overlap' chars
    ...
    def load_document(self, file_path):
        """Load and chunk a text document or PDF"""
        ...
        if file_ext == '.pdf':
            if not PDF_SUPPORT:
                print("❌ PDF support not available. Install PyPDF2: pip install PyPDF2")
                return
        ...
        if not text or len(text.strip()) < 10:
            print("❌ Document is empty or too short!")
            return
        
        # Use smart chunking with overlap
        chunks = self.smart_chunk_text(text, chunk_size=600, overlap=100)
        
        print(f"✅ Loaded {len(chunks)} chunks (chunk_size=600, overlap=100)")
...
def main():
    ...
    print("\nCommands:")
    print("  load <filename> - Load a document (.txt, .pdf)")
...
pip install google-generativeai numpy scikit-learn PyPDF2

STEP 2A: CREATE A TEST TEXT FILE (test.txt)
...
STEP 2B: OR DOWNLOAD A PDF
- Go to Wikipedia and "Print to PDF" any article
- Or use any PDF you have (research papers, books, reports)
...
✅ Same smart chunking applied

Try with:
- Research papers
- Technical documentation
- Books (if not DRM protected)
...
# 1. Download a Wikipedia article as PDF
Visit: https://en.wikipedia.org/wiki/Artificial_intelligence
File → Print → Save as PDF

# 2. Load it
> load ai_article.pdf
📄 Loading document: ai_article.pdf
📖 Extracting text from PDF...
   Found 15 pages
   Processed 10/15 pages...
✅ Extracted 45000 characters from PDF
✅ Loaded 75 chunks

# 3. Ask questions
> ask What is artificial intelligence?
> ask When did AI research begin?
> ask What are the main approaches to AI?
...
2. "Empty or too short"
   FIX: PDF might be image-based (scanned), needs OCR

3. "Error reading PDF"
   FIX: PDF might be corrupted or password-protected

4. Gibberish text extracted
   FIX: PDF encoding issue, try different PDF


=== ADVANCED: ADD MORE FILE TYPES ===

Want to read Word documents? Add this:
...
✅ Chat with PDFs (research papers, books, articles!)
